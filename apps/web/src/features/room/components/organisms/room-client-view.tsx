...
export function RoomClientView({
  ...
  const isSelfActive =
    room.round.phase === "lobby" || room.round.activePlayerIds.includes(self.id);
    ...
    <main className={clsx("mx-auto flex min-h-screen max-w-7xl flex-col gap-4 px-4 py-6", isMyanmar && "lang-my")}>
      {showVerdictReveal && room.round.phase === "results" ? (
        <div className="verdict-overlay">
          <div className={`verdict-bar ${room.round.eliminatedPlayerId ? "eliminated" : "skipped"}`}>
            <div className="mx-auto flex max-w-4xl flex-col items-center text-center">
              <p className="verdict-title text-xs font-semibold text-white/55">{t(locale, "votingComplete")}</p>
              <div className="mt-4 h-px w-24 bg-white/20" />
              <div className="mt-6 flex flex-col items-center gap-4">
                <p className={clsx("text-sm font-semibold text-white/48", isMyanmar ? "tracking-[0.05em]" : "uppercase tracking-[0.34em]")}>
                  {room.round.eliminatedPlayerId ? t(locale, "elimination") : t(locale, "noElimination")}
                </p>
                <h2 className={clsx("verdict-wordmark font-semibold", isMyanmar ? "text-3xl leading-[1.25] md:text-5xl" : "text-4xl md:text-6xl")}>
                  {room.round.eliminatedPlayerId
                    ? t(locale, "playerWasEliminated", { player: eliminatedPlayerName })
                    : t(locale, "noOneEliminated")}
                </h2>
              ...
              <div className={clsx("mt-6 rounded-full border border-white/10 bg-white/8 px-4 py-2 text-sm font-semibold text-white/72", isMyanmar ? "tracking-[0.05em]" : "uppercase tracking-[0.24em]")}>
                {room.round.eliminatedPlayerId ? t(locale, "decisionLockedIn") : t(locale, "roundSkipped")}
              </div>
            </div>
          </div>
        </div>
      ...
      <header className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="max-w-3xl">
          <div className={clsx("flex flex-wrap items-center gap-2 text-xs text-ink/50", isMyanmar ? "tracking-[0.04em]" : "uppercase tracking-[0.28em]")}>
          ...
          <h1 className={clsx("mt-2 text-ink", isMyanmar ? "text-[1.45rem] font-semibold leading-[1.4] md:text-[1.75rem]" : "font-display text-4xl leading-none md:text-5xl")}>
            {t(locale, "stayConvincing")}
          </h1>
          <p className={clsx("mt-2 max-w-2xl text-ink/70", isMyanmar ? "text-[0.98rem] leading-8" : "text-base md:text-lg")}>
        ...
        <div className="flex flex-wrap items-start gap-2 xl:max-w-[48%] xl:justify-end">
          <label className="flex min-h-[3.25rem] items-center gap-2 rounded-xl border border-black/10 bg-white/75 px-3 py-2.5 text-sm font-medium text-ink/80">
            ...
            <select
              ...
              disabled={!self.isHost || (room.round.phase !== "lobby" && room.round.phase !== "results")}
          ...
          <button onClick={shareInvite} className="min-h-[3.25rem] rounded-xl border border-black/10 bg-white/75 px-4 py-2.5 text-sm font-semibold md:text-base">
            {t(locale, "invitePlayers")}
          </button>
          <button onClick={leaveRoom} className="min-h-[3.25rem] rounded-xl border border-black/10 px-4 py-2.5 text-sm font-semibold md:text-base">
            {t(locale, "leaveRoom")}
          </button>
          {self.isHost ? (
            <button
              onClick={startRound}
              disabled={room.players.length < 3 || (room.round.phase !== "lobby" && room.round.phase !== "results")}
              className="min-h-[3.25rem] rounded-xl bg-accent px-4 py-2.5 text-sm font-semibold text-white md:text-base"
            >
              {room.round.phase === "results" ? t(locale, "startNextRound") : t(locale, "startRound")}
            </button>
          ) : null}
        </div>
      </header>
        ...
        <div className="grid gap-4">
          <Card className="bg-white/92 p-4">
            <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto_auto] md:items-center">
              <div>
                ...
                {roleState ? (
                  <button onClick={() => setSecretVisible((current) => !current)} className="mt-3 rounded-xl border border-black/10 px-3 py-2 text-sm font-semibold text-ink">
          ...
          {room.round.phase === "lobby" ? (
            <Card className="bg-white/92 p-4">
              ...
              <h2 className={clsx("mt-2 font-semibold", isMyanmar ? "text-[2rem] leading-[1.2]" : "text-2xl")}>{t(locale, "gatherSuspects")}</h2>
              <p className={clsx("mt-2 text-ink/68", isMyanmar ? "text-lg leading-8" : "")}>{t(locale, "lobbyHint")}</p>
            ...
            <Card className="space-y-3 bg-white/92 p-4">
              ...
              <div className="flex flex-col gap-3 md:flex-row md:items-stretch">
                <input
                  ...
                  className={clsx("w-full rounded-xl border border-black/10 bg-white px-4 outline-none transition focus:border-accent", isMyanmar ? "py-4 text-lg" : "py-3")}
                />
                <button
                  onClick={submitClue}
                  disabled={!isMyTurn || clue.trim().length < 1}
                  className={clsx("rounded-xl bg-ink px-4 font-semibold text-paper md:min-w-[10.5rem]", isMyanmar ? "py-4 text-lg leading-7" : "py-3")}
                >
                  {t(locale, "submitClue")}
                </button>
              </div>
          ...
          {room.round.phase === "voting" ? (
            <Card className="space-y-3 bg-white/92 p-4">
              <div>
                ...
                <h2 className={clsx("mt-1 font-semibold", isMyanmar ? "text-[2rem] leading-[1.2]" : "text-2xl")}>{t(locale, "whoSoundedOff")}</h2>
                <p className={clsx("mt-2 text-ink/68", isMyanmar ? "text-lg leading-8" : "text-sm md:text-base")}>{t(locale, "votingHint")}</p>
              </div>
                ...
                <div className="rounded-2xl border border-black/8 bg-black/[0.025] px-4 py-3">
                  ...
                  <div className="mt-2 space-y-2">
                    {activePlayers.map((player) => {
                      ...
                      return (
                        <div key={player.id} className="flex items-center justify-between rounded-xl bg-white px-3 py-2.5">
                          <div>
                            <p className="font-medium text-ink">
                              {player.nickname} {player.id === self.id ? `• ${t(locale, "you")}` : ""}
                            </p>
                            <p className="text-sm text-ink/50">
                              {vote ? t(locale, "voteSubmitted") : t(locale, "choosingNow")}
                            </p>
                          </div>
                          <span className={clsx("rounded-full px-3 py-1 text-xs font-semibold", vote ? "bg-mint/20 text-ink" : "bg-sand/35 text-ink/65", isMyanmar ? "tracking-[0.05em]" : "uppercase tracking-[0.2em]")}>
              ...
              <div className="grid gap-2">
                ...
                />
                {activePlayers.filter((player) => player.id !== self.id).map((player) => (
                  <VoteOptionRow
                    key={player.id}
              ...
              <button
                onClick={submitVote}
                disabled={!isSelfActive || Boolean(myVote)}
                className={clsx("rounded-xl bg-accent px-4 font-semibold text-white", isMyanmar ? "py-4 text-lg leading-7" : "py-3")}
              >
                {myVote
          ...
          {room.round.phase === "round-resolution" ? (
            <Card className="space-y-3 bg-white/92 p-4">
              <div className="result-stage rounded-[22px] px-4 py-4 text-paper">
                <p className={clsx("text-xs font-semibold text-paper/60", isMyanmar ? "tracking-[0.05em]" : "uppercase tracking-[0.28em]")}>
                ...
                <div className="mt-3 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <div className={clsx("rounded-full border border-white/12 bg-white/8 px-4 py-2 text-sm font-semibold text-paper/78", isMyanmar ? "tracking-[0.05em]" : "uppercase tracking-[0.24em]")}>
                ...
                <p className={clsx("mt-4 text-paper/72", isMyanmar ? "text-sm leading-7" : "text-sm")}>
                  {room.round.resolutionReason === "tie"
                    ? t(locale, "tieVoteNoElimination")
                    : room.round.eliminatedPlayerId
                      ? t(locale, "votesCastBy", { players: eliminatedByNames || t(locale, "noVotesRecorded") })
                      : t(locale, "skipVotesCastBy", { players: skippedByNames || t(locale, "noSkipVotesRecorded") })}
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-accent/20 bg-accent/8 px-4 py-3">
                ...
                {self.isHost ? (
                  <button onClick={continueResolvedRound} className="rounded-xl bg-accent px-4 py-2.5 font-semibold text-white">
          ...
          {room.round.phase === "results" ? (
            <Card className="space-y-3 bg-white/92 p-4">
              <div className="result-stage rounded-[22px] px-4 py-4 text-paper">
                ...
                <div className="mt-3 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    ...
                    <p className={clsx("mt-2 font-semibold text-white", isMyanmar ? "text-[1.7rem] leading-[1.35] md:text-[1.9rem]" : "text-3xl md:text-4xl")}>
                      {room.round.outcome?.winner === "civilians" ? t(locale, "civiliansWin") : t(locale, "undercoverWins")}
                    </p>
                  </div>
                  <div className={clsx("rounded-full border border-white/12 bg-white/8 px-4 py-2 text-sm font-semibold text-paper/78", isMyanmar ? "tracking-[0.05em]" : "uppercase tracking-[0.24em]")}>
                    {t(locale, "game")} {visibleGameNumber}
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2">
                  <div className="rounded-2xl border border-white/10 bg-white/7 px-4 py-3">
                    <p className={clsx("text-xs font-semibold text-paper/55", isMyanmar ? "tracking-[0.05em]" : "uppercase tracking-[0.24em]")}>{t(locale, "undercover")}</p>
                    <p className="mt-1 text-2xl font-semibold text-white">{revealedUndercoverName ?? t(locale, "hidden")}</p>
                  ...
                  <div className="eliminated-reveal relative overflow-hidden rounded-2xl border border-accent/30 bg-[#301f1c] px-4 py-3">
                    <p className={clsx("text-xs font-semibold text-[#f5b8ab]", isMyanmar ? "tracking-[0.05em]" : "uppercase tracking-[0.24em]")}>{t(locale, "eliminated")}</p>
              ...
              {self.isHost ? (
                <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-accent/20 bg-accent/8 px-4 py-3">
                  <div>
                    <p className="text-sm font-semibold text-ink">{t(locale, "readyForAnotherMatch")}</p>
                    <p className="text-sm text-ink/62">{t(locale, "restartHint")}</p>
                  </div>
                  <button onClick={startRound} disabled={room.players.length < 3} className="rounded-xl bg-accent px-4 py-2.5 font-semibold text-white">
                    ...
                    {room.round.votes.map((vote) => {
                      ...
                      const target =
                        vote.targetPlayerId === null
                          ? t(locale, "skip")
                          : (room.players.find((player) => player.id === vote.targetPlayerId)?.nickname ??
                            t(locale, "unknownPlayer"));

                      return (
                        <div key={vote.voterId} className="flex items-center justify-between rounded-xl bg-black/[0.03] px-3 py-2.5">
                          <div>
                            <p className="font-medium text-ink">{voter?.nickname ?? t(locale, "unknownPlayer")}</p>
                            <p className="text-sm text-ink/48">{t(locale, "finalVoteSubmitted")}</p>
                          </div>
                          <span className="rounded-full bg-sand/35 px-3 py-1 text-sm font-semibold text-ink">{target}</span>
                        </div>
                      );
                    })}
                  </div>
            ...
            <div className="mt-3 grid gap-2">
              {room.players.map((player) => (
                <PlayerStatusRow
                  ...
                  isEliminated={player.id === room.round.eliminatedPlayerId || Boolean(player.eliminatedAt)}
                  isActive={
