...
describe("RoomClient", () => {
  it("shows voting progress and the player's submitted vote state", async () => {
    ...
    await runInAct(() =>
      mockSocket.trigger("room:snapshot", {
        room: makeRoom({
          round: {
            gameNumber: 1,
            roundNumber: 1,
  ...
  it("lets the host change the word pack in the lobby", async () => {
    ...
    const updateCall = mockSocket.emitted.find((call) => call.event === "room:update-word-pack")
  ...
  it("lets the host change the room language in the lobby", async () => {
    ...
    const updateCall = mockSocket.emitted.find((call) => call.event === "room:update-locale")
    expect(updateCall?.args[0]).toEqual({
      roomCode: "ABCD",
      playerSessionId: "session-host",
      locale: "my",
